...
class GazeTrainer(Trainer):
    def __init__(self,
        ...
        self.eyediap_image_path = '/data/Eyediap_eye_center_depth/Image'
        self.eyediap_label_path = '/data/Eyediap_eye_center_depth/Label/ClusterLabel'
    ...
    def _get_eyediap_trainloader(self):
        ...
        train_loader = read_diap_with_depth_eyecenter.txtload(trainlabelpath, imagepath, self.batch_size_train,
    ...
    def _get_eyediap_valloader(self):
        ...
        test_loader = read_diap_with_depth_eyecenter.txtload(testlabelpath, imagepath, self.batch_size_train,
                                                             shuffle=True, num_workers=self.num_workers, header=True)
        return test_loader
    ...
    def _train_base_epoch(self):
        ...
        # decoder_total = sum([param.nelement() for param in decoder.parameters()])
        ...
        for i,  (data, label) in enumerate(self.temps.train_loader):
            ...
            left_gaze_gt = gaze_gt[:, :2].to(device)
            right_gaze_gt = gaze_gt[:, 2:].to(device)
            main_left_gaze_pred = total_pred[:, :2]
            main_right_gaze_pred = total_pred[:, 2:]

            left_mse_loss_coord = F.mse_loss(left_gaze_pred, left_gaze_gt)
            right_mse_loss_coord = F.mse_loss(right_gaze_pred, right_gaze_gt)
            total_pred_mse_loss_coord = F.mse_loss(total_pred, gaze_gt)
            loss_coord = L1_loss(left_right_gaze_concat, th.cat([th.cat(
                [left_gaze_gt, right_gaze_gt], 1), gaze_gt], 1))  # AWingloss(gaze_pred, gaze_gt)
            ...
            for k, gaze in enumerate(left_gaze_pred):
                ...
                left_accs_iter += self._precision(gazeto3d(gaze),
                                                  gazeto3d(left_gaze_gt.cpu().numpy()[k]))

            for k, gaze in enumerate(right_gaze_pred):
                gaze = gaze.cpu().detach().numpy()
                right_count_iter += 1
                right_accs_iter += self._precision(
                    gazeto3d(gaze), gazeto3d(right_gaze_gt.cpu().numpy()[k]))

            # for k, gaze in enumerate(total_pred):
            #     gaze = gaze.cpu().detach().numpy()
            #     total_pred_count_iter += 1
            #     total_pred_accs_iter +=  self._precision(gazeto3d(gaze), gazeto3d(gaze_gt.cpu().numpy()[k]))

            main_left_accs_iter = 0
            main_left_count_iter = 0
            for k, gaze in enumerate(main_left_gaze_pred):
                gaze = gaze.cpu().detach().numpy()
                main_left_count_iter += 1
                main_left_accs_iter += self._precision(
                    gazeto3d(gaze), gazeto3d(left_gaze_gt.cpu().numpy()[k]))
            main_left_prec_coord_iter = main_left_accs_iter/main_left_count_iter

            main_right_accs_iter = 0
            main_right_count_iter = 0
            for k, gaze in enumerate(main_right_gaze_pred):
                gaze = gaze.cpu().detach().numpy()
                main_right_count_iter += 1
                main_right_accs_iter += self._precision(
                    gazeto3d(gaze), gazeto3d(right_gaze_gt.cpu().numpy()[k]))
            main_right_prec_coord_iter = main_right_accs_iter/main_right_count_iter

            left_prec_coord_iter = left_accs_iter/left_count_iter
            right_prec_coord_iter = right_accs_iter/right_count_iter
            total_pred_prec_coord_iter = (
                main_left_prec_coord_iter + main_right_prec_coord_iter)/2
            epoch = self.temps.epoch
    ...
    def _test_base(self):
        ...
        for i, (data, label) in enumerate(self.temps.val_loader):
            ...
            with th.no_grad():
                ...
                main_left_gaze_pred = total_pred[:, :2]
                main_right_gaze_pred = total_pred[:, 2:]
                left_mse_loss_coord_iter = F.mse_loss(
                    left_gaze_pred, left_gaze_gt)
                right_mse_loss_coord_iter = F.mse_loss(
                    right_gaze_pred, right_gaze_gt)
                total_pred_mse_loss_coord_iter = F.mse_loss(
                    total_pred, gaze_gt)
                left_right_gaze_concat = th.cat(
                    [th.cat([left_gaze_pred, right_gaze_pred], 1), total_pred], 1)
                loss_coord_iter = L1_loss(left_right_gaze_concat, th.cat(
                    [th.cat([left_gaze_gt, right_gaze_gt], 1), gaze_gt], 1))
...
if __name__ == '__main__':
    ...
    model_name = "gaze_eyediap_v8_depth3_eyecentered_resnet_SEPRE_lookat_resnet_correct_left_right_feature_branch"
