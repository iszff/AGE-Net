...
class norm:  
    def __init__(self, center, leftcenter, rightcenter, gazetarget, headrotvec, imsize, camparams, newfocal=960, newdistance=600):#focal focus point
        try:
            self.center = np.array(center)
            self.left_center = leftcenter
            self.right_center = rightcenter
    ...
    def __ParamsCalculate(self):
        ...
        self.C_mat = np.array([[self.newfocal, 0, self.imsize[0]/2], [0, self.newfocal, self.imsize[1]/2], [0, 0, 1]])#shape
    ...
    def left_GetGaze(self, scale=True):
        if scale:
            left_gaze = np.dot(self.M_mat, self.left_gaze)
    ...
    def right_GetGaze(self, scale=True):
        if scale:
            right_gaze = np.dot(self.M_mat, self.right_gaze)
    ...
    def CropEyeWithCenter(self, center):
        ...
        x1 = [max(center_x - width/2, 0), max(center_y - height/2, 0)]
        x2 = [min(x1[0] + width, self.imsize[0]), min(x1[1] + height, self.imsize[1])]
        im = self.im[int(x1[1]):int(x2[1]), int(x1[0]):int(x2[0])]#h,w
